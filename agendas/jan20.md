# Team 7 Meeting #2, January 20th, 2026
We have curated research questions based on the discussions we had last team meeting. We have narrowed our potential projects down to three datasets and 6 potential research questions.

##  Datasets and Research Questions - Finalizing a Topic - Minutes by Sam Donato

### IBD Ryan
- Do sex differences in the gut microbiome modify responses to treatment to various drugs for Crohn’s disease?
- 6 drugs with enough samples and it could also be interesting to compare the treatment combinations (with and without anti-TNF)

**Comments**\
- Not much literature specifically investigating sex differences in the gut microbiome and how these differences change in response to treatment. Not specifically in terms of medications/drugs. 
- Good amount of samples and groups
- Narrow down the groups, filter
- Use alpha and beta diversity to narrow down drugs
- Find out which drugs to focus on, 1 or 2. Look at gut microbiome, sex differences and see if it acts the same with TNF and anti-TNF.

**Workflow**
- Filter for Crohn's disease
- Pre-cursor, look at literature to see what drugs affect the gut microbiome, do beta diversity
- Analyses, with and without the drug in differential abundance, taxonimical analysis
- Possibility to do random forest
- TNF, Anti-TNF as other aim

### Fish
- Does the species of fish and microbial diversity affect swimming performance and what depth they reside?
- 5 groups of different fish performances, 30+ different fish, can be a viable, novel research topic from literature review, papers showing biological and environmental differences for fish or microbial diversity for fitness of fish.

**Comments**\
The research papers cited in creating the research questions can be found here.
- https://pubmed.ncbi.nlm.nih.gov/39466622/
- https://pubmed.ncbi.nlm.nih.gov/40564368/
- Seperate slow vs. fast fish into groups
- Both viable questions
- Can't do just depth for analysis, would have to connect with swimming performance
- Key terms for fast and slow fish 

**Workflow**
- Determine the groups
- Compare broadly slow vs. fast groups (alpha and beta diversity) - 2 group comparison
- Differential abundance
- Functional analysis
- Pathway analysis
- Core microbiome
- Do other analyses not included in the course
- Issue with making it complicated enough

### Anemia during Infancy
- Does solid/liquid diet affect TCA cycle metabolites in anemic infants?
- Alpha diversity in anemic infants?

**Comments**\
We could use datasets from the Inflammatory Bowel Disease Multinomics Database (https://ibdmdb.org/results) and remove everything except the controls then use it to find bacteria that correlate to the TCA metabolites. The main downside would be that the data is from stool, not serum, but otherwise it's good quality.
